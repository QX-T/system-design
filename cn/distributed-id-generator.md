...
### 基于范围range 的预生成 ID 系统
使用一台master机器，生成所有id。它不实际生成所有的ID，而只是记录所有已经分配的ID 范围，和可用的ID范围。
在master机器下面可以有level1， level2, ..., level n级的ID服务器。
每一级服务器在本机可用的id分配完后，向上级服务器申请一个新的可用范围。上级服务器向下级服务器分配一个范围，并记录该范围已经被分配： (start, range )

叶子服务器， 直接与客户端打交道。根据从上游服务获取到的range，可以记录current，max。
max代表最大可以分配的id值，一旦current>max, 则需要从上游服务器重新申请id。
叶子服务器每次从可用id分配一个id根调用者。

通过这种方式，上游服务器只会接受少量数据请求。一旦下游服务器的请求过于频繁，则可以扩大返回的range大小，从而避免下游服务器频繁请求。例如，如果某个服务器请求过于频繁，我们将分配给该服务器的范围加大（x10)
